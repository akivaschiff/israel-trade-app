        ...
        <div v-if="availableTimeRange">
          ...
          <div class="flex items-center gap-4">
            <div class="flex-1">
              ...
              <div class="flex gap-2">
                <input
                  ...
                  type="number"
                  :min="availableTimeRange.min_year"
                  :max="availableTimeRange.max_year"
                  placeholder="Year"
                  class="flex-1 px-3 py-2 border border-gray-300 rounded-lg"
                />
                <input
                  v-model.number="startPeriod"
                  type="number"
                  min="1"
                  max="12"
                  placeholder="Month"
            ...
            <button
              ...
              class="self-end px-4 py-2 text-sm text-gray-600 hover:text-gray-900 border border-gray-300 rounded-lg hover:bg-gray-50"
            >
              Reset
            </button>
          </div>
          <div class="text-xs text-gray-500 mt-1">
            Available: {{ formatDate(availableTimeRange.min_year, availableTimeRange.min_period) }} - 
            {{ formatDate(availableTimeRange.max_year, availableTimeRange.max_period) }}
          </div>
        </div>
...
// Watch for country selection to fetch time range
watch([selectedProducts, selectedCountries, selectedFlow], async ([products, countries, flow]) => {
  if (products.size > 0 && countries.length > 0) {
    await fetchTimeRange(Array.from(products), countries, flow)
    resetTimeRange()
  }
})

// Reset time range to available range
function resetTimeRange() {
  if (availableTimeRange.value) {
    startYear.value = availableTimeRange.value.min_year
    startPeriod.value = availableTimeRange.value.min_period
    endYear.value = availableTimeRange.value.max_year
    endPeriod.value = availableTimeRange.value.max_period
  }
}

// Format date for display
function formatDate(year, period) {
